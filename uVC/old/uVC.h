...
class Directory;
class FileSystem;
const std::string pepper = "sLqxNKrO7WVn12x6Um4Mn8P2KHBrUrcbQ5lR6v2DBNu04dhVK7ml6GaDfOpFYnsu";

template <typename T, typename I>
T volatile_cast(I value);
uint64_t decompress(std::string value);
std::string compress(uint64_t value);
...
void writeBlock(char *filename, uint64_t block_addr, char buffer[BLOCK_SIZE])
{
    std::ofstream fout;
    fout.open(filename, std::ios::binary);
    fout.seekp(block_addr*BLOCK_SIZE);
    fout.write(buffer, BLOCK_SIZE);
    fout.close();
}

void ccpy(char *dst, const char *src, int dst_addr, int src_addr, int size)
{
...
FileSystem::FileSystem(std::string _key, std::string _path)
{
    ...
    if (_path != "")
    {
        ...
        else
        {
            ...
            char tmp_block[BLOCK_SIZE] = {0};
            ccpy(tmp_block,_key.c_str(), 0, 0, key.length());
            
            ccpy(tmp_block, compress(1).c_str(), 0, 256, 9);
            ccpy(tmp_block, compress(2).c_str(), 0, 265, 9);
            ccpy(tmp_block, compress(3).c_str(), 0, 274, 9);
        }
    }
}
